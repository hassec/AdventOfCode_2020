...
processRule r =
  let [k, v] = splitOn "contain" r
   in let key = unwords . init . words $ k
       in let values = map (init . words) $ filter (not . ("no other bags" `isInfixOf`)) . splitOn "," $ v
           in (key, [(count, values) | l <- values, let count = read . head $ l, let values = unwords . tail $ l])
